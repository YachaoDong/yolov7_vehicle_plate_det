...
namespace YoloFast{
    ...
    static const int NUM_BOX_ELEMENT = 7+1;      // left, top, right, bottom, confidence, class, keepflag
    ...
    static __global__ void decode_kernel(
        ...
        for(int i = 1; i < num_classes; ++i){
            ...
            // color label
            if(i >= num_classes-10){
                if(class_confidence > color_confidence){
                    color_confidence = class_confidence;
                    color_label      = i-13;
                }
            } 
        }
        ...
        // color
        color_confidence = sigmoid(color_confidence);
        color_confidence *= objectness; 
        if(color_confidence < confidence_threshold)
            return;
