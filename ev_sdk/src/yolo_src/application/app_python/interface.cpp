...
public:
	...
	shared_future<CenternetDetector::BoxArray> commit(const py::array& image){
...
PYBIND11_MODULE(libpytrtc, m) {
	py::class_<ObjectDetector::Box>(m, "ObjectBox")
        ...
        .def_property("color_label", [](ObjectDetector::Box& self){return self.color_label;}, [](ObjectDetector::Box& self, int nv){self.color_label = nv;})
		...
		.def("__repr__", [](ObjectDetector::Box& obj){
			return iLogger::format(
				"<Box: left=%.2f, top=%.2f, right=%.2f, bottom=%.2f, class_label=%d, color_label=%d,confidence=%.5f>",
				obj.left, obj.top, obj.right, obj.bottom, obj.class_label, obj.color_label, obj.confidence
			);	
		});
