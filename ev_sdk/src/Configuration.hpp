...
typedef struct
{
    ...
    bool rec_use_gpu;
    int rec_gpu_mem;
    int rec_cpu_math_library_num_threads;
    bool rec_use_mkldnn;
    std::string rec_char_dict_path;
    std::string rec_precision;    // "Precision be one of fp32/fp16/int8"
    int rec_batch_num;
    int rec_img_h;
    int rec_img_w;
} AlgoConfig;
...
struct Configuration
{
    ...
    AlgoConfig algoConfig = {0,
                             "/usr/local/ev_sdk/model/35403/my_dett.FP32.trtmodel", 0.25, 0.45,
                             "/usr/local/ev_sdk/model/34895/rec/mv3_none_bilstm_ctc/inference",
                             true, 4000, 8, true, "/usr/local/ev_sdk/src/paddle_src/src//plate_dict.txt",
                             "fp32", 1, 32, 100
                            }; // 默认的算法配置
    ...
    // 报警字段
    int alertCountThreshold = 3;
    bool vehicleClassAlert = true;       // 车辆类型控制报警
    bool vehicleColorAlert = true;       // 车辆颜色控制报警
    bool plateOcrAlert = false;            // 车牌ocr控制报警
    bool plateColorAlert = false;          // 车牌颜色控制报警
    std::vector<std::string> alertVehicleExclude = {}; //此列表中的车型不触发报警
    std::vector<std::string> alertVehicleColorExclude = {}; //此列表中的车型不触发报警
    std::vector<std::string> alertPlateChineseExclude = {}; //此列表中的车型不触发报警
    std::vector<std::string> alertPlateColorExclude = {}; //此列表中的车型不触发报警
    
    
    
    
    std::map<std::string, std::vector<std::string> > targetRectTextMap = { {"en",{"truck", "van", "car", "slagcar", "bus", "fire_truck",
        "police_car", "ambulance",  "SUV", "microbus", "unknown_vehicle", "plate", "double_plate"}}, {"zh", {"卡车","小型货车","小汽车、轿车、跑车等四轮小汽车","渣土车","公共汽车","消防车","警车","救护车","SUV、商务车等四轮小汽车","面包车","其它未知类型车辆","单层车牌","双层车牌"}}};// 检测目标框顶部文字
    
    std::map<std::string, std::vector<std::string> > targetColorTextMap = { {"en",{"white", "siliver", "grey", "black", "red", "blue", "yellow", "green", "brown", "others"}}, {"zh", {"白色","银色","灰色","黑色","红色","蓝色","黄色","绿色","棕色","其它颜色"}}}; // 检测目标框顶部文字
    
    // camera id
    std::string Cid = "Abc12345";
     ...
     * @param[in] configStr json格式的配置参数字符串  解析.json文件中的参数，将其传入此.hpp参数中通过json传参
    ...
    void ParseAndUpdateArgs(const char *confStr)
    {
        ...
        //算法相关参数
        // gpu_id
        checkAndUpdateNumber("gpu_id", algoConfig.gpu_id); 
        // yolo det
        checkAndUpdateStr("det_engine_file", algoConfig.det_engine_file);
        checkAndUpdateNumber("conf_thresh", algoConfig.conf_thresh);  
        checkAndUpdateNumber("nms_thresh", algoConfig.nms_thresh);
        // paddle ocr
        checkAndUpdateStr("rec_model_dir", algoConfig.rec_model_dir);
        checkAndUpdateBool("rec_use_gpu", algoConfig.rec_use_gpu);
        checkAndUpdateNumber("rec_gpu_mem", algoConfig.rec_gpu_mem);  
        checkAndUpdateNumber("rec_cpu_math_library_num_threads", algoConfig.rec_cpu_math_library_num_threads);
        checkAndUpdateBool("rec_use_mkldnn", algoConfig.rec_use_mkldnn);  
        checkAndUpdateStr("rec_char_dict_path", algoConfig.rec_char_dict_path);
        checkAndUpdateStr("rec_precision", algoConfig.rec_precision);  
        checkAndUpdateNumber("rec_batch_num", algoConfig.rec_batch_num);
        checkAndUpdateNumber("rec_img_h", algoConfig.rec_img_h);
        checkAndUpdateNumber("rec_img_w", algoConfig.rec_img_w);  
        ...
        // 算法报警
        checkAndUpdateNumber("alert_count_threshold", alertCountThreshold); 
        checkAndUpdateBool("vehicle_class_alert", vehicleClassAlert);
        checkAndUpdateBool("vehicle_color_alert", vehicleColorAlert);
        checkAndUpdateBool("plate_ocr_alert", plateOcrAlert);
        checkAndUpdateBool("plate_color_alert", plateColorAlert);
        
        checkAndUpdateVecStr("alert_vehicle_exclude", alertVehicleExclude);
        checkAndUpdateVecStr("alert_vehicle_color_exclude", alertVehicleColorExclude);
        checkAndUpdateVecStr("alert_plate_chinese_exclude", alertPlateChineseExclude);
        checkAndUpdateVecStr("alert_plate_color_exclude", alertPlateColorExclude);
        
        
        checkAndUpdateVecStr("mark_text_en", targetRectTextMap["en"]);        
        checkAndUpdateVecStr("mark_text_zh", targetRectTextMap["zh"]);
        // det color
        checkAndUpdateVecStr("mark_det_color_en", targetColorTextMap["en"]);        
        checkAndUpdateVecStr("mark_det_color_zh", targetColorTextMap["zh"]);  
