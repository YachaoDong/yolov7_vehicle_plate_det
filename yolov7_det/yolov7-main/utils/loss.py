...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False, nco=10):
        ...
        # color loss
        self.BCEcolor = BCEcolor
        for k in 'na', 'nc', 'nl', 'anchors':  # 这里的self.nc=23 cls+color
            setattr(self, k, getattr(det, k))
        self.nco = nco
        self.nc = self.nc - self.nco
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        tcls, tcolor, tbox, indices, anchors = self.build_targets(p, targets)  # targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    t_[range(n), tcolor[i]] = self.cp
                    # t[t==self.cp] = iou.detach().clamp(0).type(t.dtype)
                    lcolor += self.BCEcolor(ps[:, (5 + self.nc):], t_)  # BCE
    ...
    def build_targets(self, p, targets):
        ...
        gain = torch.ones(7+1, device=targets.device).long()  # normalized to gridspace gain
        ...
        for i in range(self.nl):
            ...
            c_color = t[:, 2].long().T  # class color
            gxy = t[:, 3:5]  # grid xy
            gwh = t[:, 5:7]  # grid wh
            ...
            tcls.append(c)  # class
            tcolor.append(c_color)  # class

        return tcls, tcolor, tbox, indices, anch
...
class ComputeLossOTA:
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            this_target = targets[b_idx]   # input targets(image_idx, class, color, x,y,w,h)
            ...
            for i, pi in enumerate(p):
                # p[i]: [bs, anchor_num, grid_h, grid_w, xywh + conf + classes + colors]
    ...
    def find_3_positive(self, p, targets):
        # Build targets for compute_loss(), input targets(image,class, colol, x,y,w,h)
        na, nt = self.na, targets.shape[0]  # number of anchors, targets
        indices, anch = [], []

        # color image_index+class + (color_idx) + xywh + anchor_index
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                l, m = ((gxi % 1. < g) & (gxi > 1.)).T
                j = torch.stack((torch.ones_like(j), j, k, l, m))
                t = t.repeat((5, 1, 1))[j]   # repeat 5份，分别为 上下左右和自身5个grid作准备
                offsets = (torch.zeros_like(gxy)[None] + off[:, None])[j]
            else:
                t = targets[0]
                offsets = 0
