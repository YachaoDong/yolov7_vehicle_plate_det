...
parser.add_argument('--weights', nargs='+', type=str, default='/project/train/models/exp3/weights/last.pt', help='exp3 model path(s)')
parser.add_argument('--data', type=str, default='/project/train/src_repo/yolov5_vehicle_plate_det/data/vehicle.yaml', help='(optional) dataset.yaml path')
...
def process_image(handle=None, input_image=None, args=None, **kwargs):
    ...
    # test
    # ocr_content = plate_infer_rec.ocr_predict(ocr_model, input_image)
                ...
                if int(cls)==11 or int(cls)==12:
                    ...
                    if (plate_w < opt.none_w) or (plate_h < opt.none_h) or (plate_wh < opt.none_wh):
                        ...
                        if len(plate_content) < 7 and (not isChinese(plate_content)):
                            for i in range(7-len(plate_content)):
                                if i != 0:
